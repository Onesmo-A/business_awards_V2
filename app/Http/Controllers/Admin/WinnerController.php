...
class WinnerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Pata miaka yote kutoka kwenye misimu au tumia mwaka wa sasa
        ...
        $categories = Category::whereHas('nominees', function ($q) use ($year) {
                $q->where('year', $year); // Chuja nominees wa mwaka huu tu
            })
            ->with(['nominees' => function ($query) use ($year) {
                // Panga washiriki kwa kura (nyingi juu) na chuja kwa mwaka
                $query->where('year', $year)->orderBy('votes_count', 'desc');
            }])
        ...
        return Inertia::render('Admin/Winners/Index', [
            ...
            'filters' => ['year' => $year],
            'availableYears' => $availableYears, // Tuma miaka kwa frontend
        ]);
    }
