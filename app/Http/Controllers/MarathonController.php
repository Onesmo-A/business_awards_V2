...
{
    private const MARATHON_SESSION_ORDERS_KEY = 'marathon_allowed_orders';
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $registration = MarathonRegistration::create([
                ...
                'unique_code' => 'BZ25-' . strtoupper(Str::random(6)),
            ...
            $this->rememberOrder($request, $transaction->order_id);
    ...
    public function pendingPayment(Request $request, $order_id)
    {
    ...
    public function success(Request $request)
    {
        ...
        $this->ensureOrderAccess($request, $orderId);
    ...
    public function checkStatus(Request $request)
    {
        ...
        $this->ensureOrderAccess($request, (string) $orderId);

        $transaction = Transaction::where('order_id', $orderId)
            ->where('payable_type', MarathonRegistration::class)
            ->first();
    ...
    public function findRegistration(Request $request)
    {
        ...
        $this->rememberOrder($request, $registration->transaction->order_id);
    ...
    public function showRetryPaymentPage(Request $request, $order_id)
    {
        $this->ensureOrderAccess($request, (string) $order_id);
    ...
    public function processRetryPayment(Request $request)
    {
        ...
        $this->ensureOrderAccess($request, (string) $validated['order_id']);
    ...
    {
        $orders = $request->session()->get(self::MARATHON_SESSION_ORDERS_KEY, []);
        if (!is_array($orders)) {
            $orders = [];
        }
        ...
        $request->session()->put(self::MARATHON_SESSION_ORDERS_KEY, array_slice($orders, -20));
    }
