...
class WebhookController extends Controller
{
    ...
    public function handleZenoPay(Request $request)
    {
        ...
        try {
            ...
            $payload = $request->all();
            Log::info('ZenoPay Webhook: Received and Authenticated.', [
            ...
            if (!$orderId || !$status) {
                Log::warning('ZenoPay Webhook: Missing required parameters.', [
                    'order_id' => $orderId,
            ...
            DB::transaction(function () use ($transaction, $status, $payload) {
                ...
                if ($status === 'COMPLETED') {
                    ...
                    // BORESHO: Pata admin wote ili kuwatumia notisi
                    ...
                    if ($transaction->payable instanceof MarathonRegistration) {
                        ...
                        if ($transaction->payable->email) {
                            Mail::to($transaction->payable->email)->queue(new MarathonPaymentCompleted($transaction->payable));
                        }
                        // Tuma notisi kwa admin
                        if ($admins->isNotEmpty()) {
                            $admins->each->notify(new NewMarathonRegistration($transaction->payable));
                            Log::info('ZenoPay Webhook: Queued marathon registration notification for admin(s).', ['count' => $admins->count()]);
                        } else {
                            Log::warning('ZenoPay Webhook: No admin users found to notify for new marathon registration.');
                        }
                        ...
                        Mail::to($transaction->payable->applicant_email)->queue(new NomineePaymentCompleted($transaction->payable));
                        ...
                        // Tuma notisi kwa admin
                        if ($admins->isNotEmpty()) {
                            $admins->each->notify(new NewNomineeApplication($transaction->payable));
                            Log::info('ZenoPay Webhook: Queued nominee application notification for admin(s).', ['count' => $admins->count()]);
                        } else {
                            Log::warning('ZenoPay Webhook: No admin users found to notify for new nominee application.');
                        }
                        ...
                        if ($admins->isNotEmpty()) {
                            $admins->each->notify(new NewTicketPurchase($purchase));
                            Log::info('ZenoPay Webhook: Queued ticket purchase notification for admin(s).', ['count' => $admins->count()]);
                        } else {
                            Log::warning('ZenoPay Webhook: No admin users found to notify for new ticket purchase.');
                        }
                    }
                    ...
                    if ($transaction->payable) {
                        ...
                        if ($transaction->payable instanceof MarathonRegistration && $transaction->payable->email) {
                            ...
                            Mail::to($transaction->payable->email)->queue(new MarathonPaymentFailed($transaction->payable));
                        } elseif ($transaction->payable instanceof NomineeApplication) {
                            $transaction->payable->update(['status' => NomineeApplication::STATUS_PAYMENT_FAILED]);
                            Mail::to($transaction->payable->applicant_email)->queue(new NomineePaymentFailed($transaction->payable));
