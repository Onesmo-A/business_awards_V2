...
{
    private const TICKET_SESSION_ORDERS_KEY = 'ticket_allowed_orders';
    ...
    public function accountIndex(): Response
    {
        ...
        $purchases = TicketPurchase::query()
            ...
            ->latest()
            ->get()
            ->map(function (TicketPurchase $purchase) {
                return [
                    'id' => $purchase->id,
                    ...
                    'created_at' => optional($purchase->created_at)?->format('d M Y'),
                    'ticket_type' => $purchase->ticketType ? [
                        'id' => $purchase->ticketType->id,
                        'name' => $purchase->ticketType->name,
                    ] : null,
                    'transaction' => $purchase->mainTransaction ? [
                        'order_id' => $purchase->mainTransaction->order_id,
                        'status' => $purchase->mainTransaction->status,
                    ] : null,
                    'tickets' => $purchase->tickets->map(fn ($ticket) => [
                        'id' => $ticket->id,
                        'ticket_code' => $ticket->ticket_code,
                        'checked_in_at' => optional($ticket->checked_in_at)?->format('d M Y H:i'),
                    ])->values(),
                ];
            });
        ...
        return Inertia::render('User/Tickets/Index', [
            ...
            'purchases' => $purchases,
            'ticketsAnnounced' => $ticketTypes->isNotEmpty(),
        ]);
    }
    ...
    'title' => 'Purchase Tickets for the Business Awards',
    'description' => 'Choose your ticket type and be part of a historic evening recognizing outstanding business achievements.',
    'ticketTypes' => $ticketTypes,
]);
    }
    ...
    public function processPurchase(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            $this->rememberOrder($request, $orderId);
     ...
     */
    public function pendingPayment(Request $request, string $order_id): Response
    {
        $this->ensureOrderAccess($request, $order_id);
     ...
     */
    public function success(Request $request, string $order_id): Response
    {
        $this->ensureOrderAccess($request, $order_id);
        ...
        $safePurchase = [
            ...
            'transaction' => [
                'order_id' => $purchase->transaction?->order_id,
            ],
        ];
    ...
    public function checkStatus(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $this->ensureOrderAccess($request, $validated['order_id']);
     ...
     */
public function downloadPdf(Request $request, string $order_id)
{
    $this->ensureOrderAccess($request, $order_id);
    ...
    return $pdf->download('business-Awards-Tickets.pdf');
}

private function rememberOrder(Request $request, string $orderId): void
{
    $orders = $request->session()->get(self::TICKET_SESSION_ORDERS_KEY, []);
    if (!is_array($orders)) {
        $orders = [];
    }
    ...
    $request->session()->put(self::TICKET_SESSION_ORDERS_KEY, array_slice($orders, -20));
}

private function ensureOrderAccess(Request $request, string $orderId): void
{
    $orders = $request->session()->get(self::TICKET_SESSION_ORDERS_KEY, []);
    abort_if(!is_array($orders) || !in_array($orderId, $orders, true), 403);
}
