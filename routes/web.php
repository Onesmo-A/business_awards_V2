...
use App\Http\Controllers\SponsorshipPackageController as PublicSponsorshipPackageController;
...
use App\Http\Controllers\Admin\SeasonAwardController;
use Inertia\Inertia;
...
// ========== Storage File Serving Routes (PUBLIC - No Auth Required) ==========
// Files are served from storage/app/public directory
// This works on all servers (localhost, shared, dedicated, cloud)
Route::get('/file/{path}', [StorageController::class, 'serve'])
    ->where('path', '.*')
    ->name('storage.file');

Route::get('/stream/{path}', [StorageController::class, 'stream'])
    ->where('path', '.*')
    ->name('storage.stream');
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::prefix('dashboard')->name('user.')->group(function () {
        ...
        Route::put('/applications/{application}', [NomineeApplicationController::class, 'update'])->name('applications.update');
        Route::get('/tickets', [TicketController::class, 'accountIndex'])->name('tickets.index');
        // Route hii ni muhimu kwa ajili ya kujaribu kulipa tena
    ...
    Route::middleware(['admin'])->prefix('admin')->name('admin.')->group(function () {
        ...
        Route::get('/votes/attempt-logs', [AdminVoteAttemptLogController::class, 'index'])->name('votes.attempt-logs.index');
        Route::get('/votes/archive', [AdminVoteController::class, 'archiveIndex'])->name('votes.archive.index');
        Route::post('/votes/archive', [AdminVoteController::class, 'archiveStore'])->name('votes.archive.store');
        ...
        Route::get('/applications/export/photos', [AdminNomineeApplicationController::class, 'exportPhotos'])->name('applications.export.photos');
        ...
        // BORESHO: Ongeza routes za kusimamia Wadhamini (Sponsors)
        Route::resource('sponsors', AdminSponsorController::class)->except(['show'])->names('sponsors');

        // BORESHO: Ongeza routes za kusimamia Packages za Udhamini
        Route::resource('sponsorship-packages', \App\Http\Controllers\Admin\SponsorshipPackageController::class)->names('sponsorship-packages');

        // BORESHO: Ongeza routes za kusimamia Misimu (Seasons)
        Route::resource('seasons', SeasonAwardController::class)->except(['show'])->names('seasons');

        // BORESHO: Ongeza routes za kusimamia Hero Section
        Route::resource('hero-sections', \App\Http\Controllers\Admin\HeroSectionController::class)->except(['show'])->names('hero-sections');

        // BORESHO: Ongeza routes za kusimamia Map Section (Event Location)
        Route::resource('map-sections', \App\Http\Controllers\Admin\MapSectionController::class)->except(['show'])->names('map-sections');

        // Guest Of Honor Management
        Route::resource('guest-of-honor', \App\Http\Controllers\Admin\GuestOfHonorController::class)->except(['show'])->names('guest-of-honor');

        // Vote exports
        // Routes hizi sasa zinafungua kurasa (Views)
        Route::get('votes/export/pdf', [\App\Http\Controllers\Admin\VoteController::class, 'exportPdf'])->name('votes.export.pdf');
        Route::get('votes/export/summary', [\App\Http\Controllers\Admin\VoteController::class, 'exportSummary'])->name('votes.export.summary');
        // Routes mpya za kupakua faili (Download)
        Route::get('votes/download/pdf', [\App\Http\Controllers\Admin\VoteController::class, 'downloadPdf'])->name('votes.download.pdf');
        Route::get('votes/download/summary', [\App\Http\Controllers\Admin\VoteController::class, 'downloadSummary'])->name('votes.download.summary');
...
Route::controller(TicketController::class)->prefix('tickets')->name('tickets.')->group(function () {
    ...
    Route::get('/status', 'checkStatus')->middleware('throttle:30,1')->name('status'); // Endpoint ya AJAX kuangalia status
});
...
// BORESHO: Route ya kuonyesha sponsorship packages kwa umma
Route::get('/sponsorship-packages', [PublicSponsorshipPackageController::class, 'index'])->name('sponsorship-packages.index');


// Awards Pages (legacy or other routes)
Route::get('/awards', [AwardsController::class, 'index'])->name('awards.index');
// Route::get('/awards/winners', [AwardsController::class, 'winners'])->name('awards.winners'); // IMEONDOLEWA: Haina matumizi tena
// BORESHO: Ongeza route ya kuonyesha washindi kwa msimu maalum (mwaka)
Route::get('/awards/seasons/{year}', [AwardsController::class, 'seasonShow'])->name('awards.seasons.show');
...
Route::prefix('marathon')->name('marathon.')->group(function () {
    ...
    Route::post('/check-status', [MarathonController::class, 'checkStatus'])->middleware('throttle:20,1')->name('check-status'); // AJAX endpoint
...
Route::get('/invitation/{uuid}', [\App\Http\Controllers\InvitationController::class, 'show'])
    ->name('invitation.show');
Route::post('/invitation/{uuid}/rsvp', [\App\Http\Controllers\InvitationController::class, 'rsvp'])
...
// Inalindwa na 'throttle:votes' (kutoka AppServiceProvider) bila kutegemea IP pekee.
