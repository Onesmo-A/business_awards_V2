...
const isSponsorsDropdownOpen = ref(false);
...
const sponsorsMegaMenu = [
    {
        title: 'Partnerships',
        links: [
            { name: 'Our Sponsors', href: route('sponsors.index'), description: 'View our esteemed partners and sponsors.' },
            { name: 'Sponsorship Packages', href: route('sponsorship-packages.index'), description: 'Explore partnership opportunities and benefits.' },
        ]
    }
];
...
const closeAllDropdowns = () => {
  ...
  isSponsorsDropdownOpen.value = false;
};
...
<template>
 <header
  :class="[
    'fixed top-10 left-0 right-0 z-40 transition-all duration-300 ease-in-out overflow-x-clip',
    isScrolled
  ...
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-20">
        ...
        <nav class="hidden lg:flex items-center space-x-1 ml-auto">
          ...
          <button
            @mouseover="isSponsorsDropdownOpen = true; isAwardsDropdownOpen = false; isMediaDropdownOpen = false"
            class="group inline-flex items-center px-3 py-2 font-medium transition text-[var(--text-primary)] hover:text-[var(--text-accent)]"
          >
            <span>Sponsors</span>
            <ChevronDownIcon class="ml-2 h-5 w-5 transition-transform" :class="{ 'rotate-180': isSponsorsDropdownOpen }" />
          </button>
        ...
        <div class="flex lg:hidden">
          <button
            ...
            class="inline-flex items-center justify-center p-2 rounded-md text-[var(--text-primary)] hover:text-[var(--text-accent)] hover:bg-white/5 focus:outline-none"
          >
    ...
    <transition name="fade">
      ...
      >
        <Link
          ...
          class="block px-3 py-2 rounded-md text-base font-medium text-[var(--text-primary)] hover:text-[var(--text-accent)] hover:bg-white/5"
        >
          {{ item.name }}
        </Link>
        ...
        <div>
          <button
            @click="mobileSubMenuOpen = mobileSubMenuOpen === 'sponsors' ? '' : 'sponsors'"
            class="w-full flex items-center justify-between px-3 py-2 rounded-md text-base font-medium text-[var(--text-primary)] hover:text-[var(--text-accent)] hover:bg-white/5"
          >
            <span>Sponsors</span>
            <ChevronDownIcon class="h-5 w-5 transition-transform" :class="{ 'rotate-180': mobileSubMenuOpen === 'sponsors' }" />
          </button>
          <div v-show="mobileSubMenuOpen === 'sponsors'" class="pt-2 pb-1 pl-4 space-y-3 bg-black/5 dark:bg-black/20 rounded-lg">
            <div v-for="column in sponsorsMegaMenu" :key="column.title">
                <Link
                    v-for="item in column.links"
                    :key="item.name"
                    :href="item.href"
                    @click="closeMobileMenu"
                    class="block px-3 py-2 rounded-md text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-accent)] hover:bg-white/5 ml-2"
                >
                    {{ item.name }}
                </Link>
            </div>
        ...
        <div>
          ...
          <div v-show="mobileSubMenuOpen === 'awards'" class="pt-2 pb-1 pl-4 space-y-3 bg-black/5 dark:bg-black/20 rounded-lg">
            <div v-for="column in awardsMegaMenu" :key="column.title">
                <p class="px-3 pt-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ column.title }}</p>
                <Link
                    v-for="item in column.links"
                    :key="item.name"
                    :href="item.href"
        ...
        <div>
          ...
          <div v-show="mobileSubMenuOpen === 'media'" class="pt-2 pb-1 pl-4 space-y-3 bg-black/5 dark:bg-black/20 rounded-lg">
        ...
        <div class="border-t border-gray-200 pt-4 pb-3 space-y-2">
          <template v-if="isAuthenticated && user">
            ...
            <button
              @click="handleLogout"
              class="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-[var(--text-primary)] hover:text-[var(--text-accent)] hover:bg-white/5"
            >
              Log Out
            </button>
          </template>
          <template v-else>
            <Link
              :href="route('login')"
              @click="closeMobileMenu"
              class="block btn-primary px-3 py-2 rounded-md text-base font-medium text-[var(--text-primary)] hover:text-[var(--text-accent)] hover:bg-white/5"
            >
              Sign In
            </Link>
            <Link
              :href="route('register')"
              @click="closeMobileMenu"
              class="btn-secondary block px-3 py-2 rounded-md text-base font-medium text-[var(--text-primary)] hover:text-[var(--text-accent)] hover:bg-white/5"
            >
              Sign Up
            </Link>
          </template>
    ...
    <!-- Sponsors Mega Dropdown -->
    <transition name="fade">
      <div v-show="isSponsorsDropdownOpen" class="absolute left-0 top-20 w-screen z-40" @mouseleave="isSponsorsDropdownOpen = false">
            ...
            <div v-for="column in sponsorsMegaMenu" :key="column.title">
              ...
              <ul class="space-y-4">
                <li v-for="item in column.links" :key="item.name">
                  ...
                  >
                    <p class="font-semibold text-[var(--text-accent)] text-lg group-hover:text-[var(--accent-secondary)]">{{ item.name }}</p>
