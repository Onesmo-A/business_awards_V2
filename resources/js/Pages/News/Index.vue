...
const getImageUrl = (path) => {
    ...
    // FIX: Ikiwa path inatoka backend ikiwa na '/storage/', ibadilishe iwe '/file/storage/'
    // Hii inashughulikia URL kamili au relative paths zinazotoka kwenye backend
    if (path.includes('/storage/')) {
        return path.replace('/storage/', '/file/storage/');
    }

    // Kama path ni URL kamili (http...) au inaanza na /, itumie hivyo hivyo
    if (path.startsWith('http') || path.startsWith('/')) return path;
    
    // Hapa tunaunganisha jina la faili na route ya controller yenu (`/file/`)
    // kama ilivyoainishwa kwenye routes/web.php
    return `/file/${path}`;
};
</script>
...
<template>
  ...
  <PageHeader 
    :title="title "
    subtitle="Taarifa na matangazo kutoka Business Awards."
  class="mx-auto max-w-7xl px-6 lg:px-8 py-16 sm:py-24"/>
