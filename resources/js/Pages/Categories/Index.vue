...
const runSearch = debounce((value) => {
  ...
  router.get(route('categories.index'), { search: trimmedSearch || undefined }, {
    preserveState: true,
    preserveScroll: true,
    replace: true,
    only: ['categoryGroups', 'filters', 'year'],
    showProgress: false,
...
<template>
<DefaultLayout>
  <Head title="business awards Categories" />
        ...
        <div class="mx-auto max-w-xl py-6">
          <div class="relative">
            <input
              ...
              name="search"
              id="search"
              class="block w-full rounded-xl bg-gray-900/80 border border-gray-700 py-3 pl-12 pr-12 text-white placeholder-gray-500 focus:ring-2 focus:ring-[var(--accent-primary)] transition shadow-inner"
            ...
            <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none">
              <button
                ...
                @click="search=''"
                type="button"
                class="text-gray-400 hover:text-gray-900 focus:outline-none pointer-events-auto"
              >
          ...
          <p v-if="loading" class="mt-2 text-xs text-gray-400">Searching categories...</p>
        </div>
      </div>
    </div>
        ...
        <div v-if="categoryGroups.meta && categoryGroups.meta.last_page > 1" class="mt-16 flex justify-center items-center gap-2">
          <div v-for="(link, key) in categoryGroups.meta.links" :key="key">
            <Link
              ...
              :href="link.url"
              v-html="link.label"
              class="px-4 py-2 text-sm font-medium rounded-full transition-colors hover:bg-yellow-600/20 hover:text-yellow-400"
