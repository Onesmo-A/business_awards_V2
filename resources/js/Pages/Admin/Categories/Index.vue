...
const props = defineProps({
  categories: Object,
  filters: Object,
  availableYears: Array, // Pokea miaka
...
const selectedYear = ref(props.filters.year || props.currentYear);

watch([search, selectedYear], debounce(([searchValue, yearValue]) => {
  router.get(route('admin.categories.index'), { search: searchValue, year: yearValue }, {
    preserveState: true,
    replace: true,
  });
}, 300));
  ...
  <div class="p-6 text-gray-100">
    ...
    <div class="flex justify-between items-center mb-6">
      ...
      <div class="flex items-center space-x-2 md:space-x-3">
        ...
        <Link
          :href="route('admin.categories.create', { year: selectedYear })"
    ...
    <div class="mb-4 flex flex-col md:flex-row gap-4">
      <div class="w-full md:w-1/3">
        <input
          ...
          class="block w-full px-4 py-2 border border-gray-600 rounded-md shadow-sm
      ...
      <table class="w-full text-sm text-left text-gray-300">
        <thead class="text-xs text-gold-400 uppercase bg-gray-700/50">
          <tr>
            ...
            <th class="px-6 py-3">Nominees ({{ selectedYear }})</th>
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-6 py-4">
              <!-- BORESHO: Onyesha 'Free' kama kategoria haina malipo -->
              {{ (category.is_free || category.nomination_fee == 0) ? 'Free' : (category.parent_id ? Number(category.nomination_fee).toLocaleString() : 'N/A') }}
            </td>
