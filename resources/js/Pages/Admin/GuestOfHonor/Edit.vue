...
const form = useForm({
    ...
    bio: props.guest.bio,
    image: null,
    is_active: Boolean(props.guest.is_active),
});

const submit = () => {
    form.post(route('admin.guest-of-honor.update', props.guest.id));
};
</script>

<template>
    <Head title="Edit Guest of Honor" />
    <AdminLayout>
        <div class="max-w-2xl mx-auto bg-gray-800 p-6 rounded-lg border border-gray-700 shadow-lg">
            <h2 class="text-xl font-bold text-white mb-6">Edit Guest of Honor</h2>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div>
                    <InputLabel for="image" value="Photo (Leave empty to keep current)" class="text-gray-300" />
                    <input type="file" @input="form.image = $event.target.files[0]" class="mt-1 block w-full text-gray-300" />
                    <InputError :message="form.errors.image" class="mt-2" />
                    <div v-if="guest.image_url" class="mt-2">
                        <img :src="guest.image_url" class="h-20 rounded border border-gray-600" />
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <button type="submit" class="btn-primary" :disabled="form.processing">Update Guest</button>
                </div>
            </form>
        </div>
    </AdminLayout>
