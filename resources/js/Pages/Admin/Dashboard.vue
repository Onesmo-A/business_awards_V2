...
watch(selectedYear, (newYear) => {
    router.get(route('admin.dashboard'), { year: newYear }, {
        preserveState: true,
        preserveScroll: true,
        replace: true
    });
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
      <div>
        ...
        <p class="mt-1 text-sm text-red-100">Your control center for the Business Awards.</p>
      </div>
      
      <div class="mt-4 sm:mt-0 flex items-center space-x-4">
          <div class="w-32">
            <select v-model="selectedYear" class="block w-full rounded-md border-gray-600 bg-gray-800 text-white shadow-sm focus:border-gold-500 focus:ring-gold-500 sm:text-sm">
                <option v-for="year in availableYears" :key="year" :value="year">{{ year }}</option>
            </select>
          </div>
          <button @click="refreshData" class="inline-flex items-center px-4 py-2 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-lg transition text-sm shadow-md">
    ...
    <!-- Welcome Message -->
    <div class="bg-gray-800/50 border border-gold-500/10 text-white p-6 rounded-xl shadow-lg">
      <h3 class="text-lg font-medium">
        Welcome to <span class="text-gold-400 font-semibold">Business Awards</span> Admin Dashboard!
      </h3>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-8 gap-6">
      ...
      <StatCard title="New Suggestions" :value="stats.suggestions" :href="route('admin.suggestions.index')" color="gray">
        <SuggestionIcon class="h-6 w-6" />
      </StatCard>

      <!-- BORESHO: Ongeza kadi za maombi yaliyokubaliwa na kukataliwa -->
      <StatCard title="Approved Applications" :value="stats.approved_applications" :href="route('admin.applications.index', { status: 'approved' })" color="green">
        <CheckCircleIcon class="h-6 w-6" />
      </StatCard>

      <StatCard title="Rejected Applications" :value="stats.rejected_applications" :href="route('admin.applications.index', { status: 'rejected' })" color="red">
        <XCircleIcon class="h-6 w-6" />
      </StatCard>
      <!-- Mwisho wa Boresho -->
      
      <StatCard title="Total Posts" :value="stats.posts" :href="route('admin.posts.index')" color="gray">
      ...
      <StatCard title="Total Categories" :value="stats.categories" :href="route('admin.categories.index')" color="gray">
        <CategoryIcon />
      </StatCard>

      <StatCard title="Total Nominees" :value="stats.nominees" :href="route('admin.nominees.index')" color="gray">
        <NomineeIcon />
      </StatCard>
      ...
      <StatCard title="Vote Issues Today" :value="stats.vote_attempt_issues_today" :href="route('admin.votes.attempt-logs.index')" color="yellow">
        <ExclamationTriangleIcon class="h-6 w-6" />
      </StatCard>

      <StatCard title="Total Users" :value="stats.users" :href="route('admin.users.index')" color="gray">
        <UserIcon class="h-6 w-6" />
      </StatCard>

      <StatCard title="Tickets Sold" :value="stats.tickets_sold" :href="route('admin.dashboard')" color="gray">
        <TicketIcon class="h-6 w-6" />
      </StatCard>

      <!-- BORESHO: Ongeza kadi za Marathon -->
      <StatCard title="Marathon Registrations" :value="stats.marathon_total" :href="route('admin.marathon.index')" color="gray">
          <TicketIcon class="h-6 w-6" />
      </StatCard>
