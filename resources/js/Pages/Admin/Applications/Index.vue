...
<div class="flex space-x-2">
    <a :href="route('admin.applications.export.pdf', { search: filters.search, status: filters.status })" target="_blank" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
        Export PDF
    </a>
    <a :href="route('admin.applications.export.photos', { search: filters.search, status: filters.status })" target="_blank" class="inline-flex items-center rounded-md bg-green-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-500">
        Export Photos ZIP
    </a>
</div>
                </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-300">
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    <tr v-for="application in applications.data" :key="application.id">
                                        <td class="max-w-[140px] truncate overflow-hidden py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6" :title="application.applicant_name">{{ application.applicant_name_display }}</td>
                                        <td class="max-w-[140px] truncate overflow-hidden px-3 py-4 text-sm text-gray-500" :title="application.business_name || '-'">{{ application.business_name_display || '-' }}</td>
                                        <td class="max-w-[160px] truncate overflow-hidden px-3 py-4 text-sm text-gray-500" :title="application.category?.name || 'Unknown'">{{ application.category?.name || 'Unknown' }}</td>
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            <!-- BORESHO: Onyesha kama ni bure au kiasi cha malipo -->
                                            <div v-if="application.transaction">
                                                <p class="font-semibold text-gray-800">
                                                    {{ new Intl.NumberFormat('en-US', { style: 'currency', currency: 'TZS' }).format(application.transaction.amount) }}
                                                </p>
                                            ...
                                            <span v-else class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset bg-gray-50 text-gray-600 ring-gray-500/10">
                                                Free Application
                                            </span>
                                        </td>
                                    ...
                                    <tr v-if="applications.data.length === 0">
                                        <td colspan="8" class="px-6 py-4 text-center text-sm text-gray-500">No applications found.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
