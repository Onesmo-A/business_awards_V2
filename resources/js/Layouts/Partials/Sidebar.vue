...
, CalendarDaysIcon } from '@heroicons/vue/24/outline'; // BORESHO: Ongeza icon ya marathon na seasons
...
const isItemActive = (item) => {
    if (item.children && item.children.length > 0) {
        return item.children.some(child => child.activePattern && route().current(child.activePattern));
    }
    return item.activePattern ? route().current(item.activePattern) : (item.route ? route().current(item.route) : false);
};
</script>
                    ...
                    <div v-for="item in menuItems" :key="item.name">
                        <component :is="item.children.length ? 'button' : Link"
                                   ...
                                   :class="[
                                       ...
                                       isItemActive(item) ? 'text-white bg-gray-800' : 'text-gray-400 hover:bg-gray-800 hover:text-white'
                                ]"
                                   :title="!(sidebarOpen || mobileSidebarOpen) ? item.name : ''">
                            <span v-if="isItemActive(item)" class="absolute inset-y-0 left-0 w-1 bg-gold-500 rounded-r-lg"></span>
                            <component :is="item.icon" class="h-6 w-6 flex-shrink-0" :class="[isItemActive(item) ? 'text-gold-400' : 'text-gray-500', sidebarOpen || mobileSidebarOpen ? 'w-6' : 'w-7']" />
